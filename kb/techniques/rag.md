...
category: technique
tags: [retrieval, knowledge, claude]
sources:
  - https://www.promptingguide.ai/techniques/rag
  - https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices
when_to_use: |
  Knowledge-grounded tasks where the answer must come from your data:
  product docs, internal wiki, customer history, policies. Anywhere
  hallucination risk is high and ground truth exists.
when_not_to_use: |
  General-knowledge tasks. Tasks where the model already knows the
  answer (you're adding cost without improving accuracy). Tasks where
  the corpus is small enough to fit in context directly.
claude_notes: |
  Combine retrieval with document-first ordering: <documents> at the
  top, <task> at the bottom. Index each doc with attributes
  (`<document index="1" source="...">`) so Claude can cite. Stable
  prefix → effective prompt caching. Always specify the escape hatch:
  "If the documents don't contain the answer, say so."
related: [document-first-ordering, generated-knowledge, prompt-chaining]
---
