...
when_to_use: |
  Tasks requiring tool use or multi-step lookups: research, retrieval,
  computation, navigation. Anywhere the model needs to act, observe a
  result, and decide what to do next.
when_not_to_use: |
  Single-hop tasks. Tasks where you can pre-fetch the data and pass it
  directly. ReAct adds latency per step; if no acting is needed, skip it.
claude_notes: |
  On Claude, ReAct typically takes the form of native tool use: declare
  tools, let Claude call them, feed results back as tool_result messages.
  For prompt-only ReAct (no native tools), structure each step as
  Thought → Action → Observation, then loop. Pair with extended thinking
  on Opus 4.7 for hard planning steps.
related: [chain-of-thought, prompt-chaining, function-calling]
---
# ReAct Prompting
