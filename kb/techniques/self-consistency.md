...
category: technique
tags: [reasoning, ensembling]
sources:
  - https://www.promptingguide.ai/techniques/consistency
  - https://arxiv.org/abs/2203.11171
when_to_use: |
  Multi-step reasoning where a single chain can lock in early errors —
  math, logic puzzles, classification with hidden criteria. Ensembling
  CoT samples and taking a majority vote materially raises accuracy.
when_not_to_use: |
  Tasks with deterministic answers (extraction, formatting). Tight
  latency budgets. Already-solved-zero-shot tasks. Cost scales N×.
claude_notes: |
  Sample N reasoning chains by setting temperature ~0.7 and running N
  parallel calls (or N branches in one orchestration). Aggregate the
  final answers (not the chains) and majority-vote. Pair with
  extended-thinking budget per chain for hard problems.
related: [chain-of-thought, tree-of-thoughts]
---
# Self-Consistency
