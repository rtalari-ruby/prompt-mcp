...
category: technique
tags: [composition, pipeline, agent]
sources:
  - https://www.promptingguide.ai/techniques/prompt-chaining
  - https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices
when_to_use: |
  Tasks too complex for one prompt: extraction → reasoning → formatting,
  research → summarize → critique, retrieve → answer → verify. Each step
  has its own narrow contract.
when_not_to_use: |
  Tasks one well-built prompt can do. Latency-sensitive paths (each step
  is a roundtrip). Tasks where intermediate state is hard to validate.
claude_notes: |
  Each step gets its own role and output contract. Pass step N's output
  into step N+1 as a tagged region (`<previous_step_output>...`). Use
  prompt caching on stable system prompts shared across steps. For an
  agent variant, see techniques/react.
related: [react, chain-of-thought, prompt-chaining-claude]
---
# Prompt Chaining
